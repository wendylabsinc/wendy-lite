...
static const ble_uuid128_t s_prov_svc_uuid =
    BLE_UUID128_INIT(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                     0x01, 0x00, 0x59, 0x44, 0x4e, 0x45, 0x57, 0x4e);
...
esp_err_t wendy_ble_prov_init(const wendy_ble_prov_callbacks_t *callbacks)
{
    ...
    {
        extern esp_err_t wendy_cloud_prov_register_gatt(void);
        esp_err_t cloud_err = wendy_cloud_prov_register_gatt();
        if (cloud_err != ESP_OK) {
            ESP_LOGW(TAG, "cloud prov GATT registration failed");
        }
    }
#endif
