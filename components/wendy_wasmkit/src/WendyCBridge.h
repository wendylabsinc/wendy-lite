...
// Version/MAC formatters (defined in WasmKitBridge.c).
// Avoid variadic snprintf which is unavailable in Embedded Swift.
extern int wendy_format_version(char *buf, int len);
extern int wendy_format_mac(char *buf, int len, const uint8_t *mac);
...
// Heap stats logger (defined in WasmKitBridge.c)
extern void wendy_log_heap(void);
