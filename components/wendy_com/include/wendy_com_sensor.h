...
// Streams sensor data to a client as sensor_frame messages. One stream and
// one in-flight frame at a time. All functions must be called from the com
// task; wendy_core offers the same three, callable from any task.
...
///
/// done runs on the com task once the frame has been sent or the link died
/// under it — either way the buffer is free again. It must not call back into
/// wcom_sensor_*: the tx queue may be tearing down around it, and a frame
/// queued from there would be orphaned. Go through
...
/// Close the stream. A frame already queued still completes, and its done
/// callback still runs.
