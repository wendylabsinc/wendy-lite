 ...
 * WAMR signature "(*~)i" means the runtime auto-converts the pointer
...
esp_err_t wendy_wasm_init(const wendy_wasm_config_t *config)
{
    ...
    /* Built-in native symbols (wendy_print, etc.) */
    init_args.n_native_symbols  = sizeof(s_builtin_symbols) / sizeof(s_builtin_symbols[0]);
    init_args.native_symbols    = s_builtin_symbols;
    init_args.native_module_name = "wendy";
                                         ...
                                         wendy_wasm_module_handle_t *out)
{
    ...
    uint8_t *buf = wamr_malloc(wasm_len);
    if (!buf) {
        return ESP_ERR_NO_MEM;
    }

    err = esp_partition_read(part, 4, buf, wasm_len);
    if (err != ESP_OK) {
        wamr_free(buf);
        return err;
    }

    err = wendy_wasm_load(buf, wasm_len, out);
    wamr_free(buf); /* wendy_wasm_load makes its own copy */
    return err;
}
